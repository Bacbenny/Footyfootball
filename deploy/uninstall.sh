...
if [[ $EUID -ne 0 ]]; then
  echo "Cần chạy với sudo: sudo bash deploy/uninstall.sh" >&2
  exit 1
fi

log "Dừng và xoá dịch vụ…"
systemctl stop footyfootball-scraper.service footyfootball-scraper.timer \
  footyfootball-web.service 2>/dev/null || true
systemctl disable footyfootball-web.service footyfootball-scraper.timer \
  footyfootball-scraper.service 2>/dev/null || true
